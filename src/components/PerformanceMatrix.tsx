  ...
  return (
    <div className="animate-in zoom-in-95 duration-700">
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 sm:p-6 lg:p-10 border border-slate-100 dark:border-slate-700 shadow-xl relative overflow-visible flex flex-col gap-5">
        ...
        {/* Score ring + suggestion */}
        <div className="flex flex-col sm:flex-row items-center sm:items-stretch gap-5">
          {/* Ring */}
          <div className="shrink-0 flex flex-col items-center justify-center">
            <svg className="w-36 h-36 sm:w-40 sm:h-40 lg:w-48 lg:h-48" viewBox="0 0 100 100">
              ...
              {isValidated ? (
                <>
                  ...
                  <text x="50" y="52" textAnchor="middle" fill="currentColor" dominantBaseline="middle" style={{ fontSize: 14, fontWeight: 900 }}>
            ...
            <p className="mt-2 text-[10px] font-black text-blue-600 uppercase tracking-[0.3em]">
              {isValidated ? 'KPI POINTS' : hasUserPending ? 'Audit Active' : 'Standby Mode'}
            </p>
          </div>

          {/* Suggestion / empty state */}
          {quarterlyStats ? (
            <div className="flex-1 min-w-0 bg-slate-50 dark:bg-[#0b1222]/50 p-4 sm:p-6 rounded-xl border border-slate-100 dark:border-slate-700 flex flex-col justify-center relative overflow-hidden">
              <div className="relative z-10 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  <Sparkles className={`w-4 h-4 sm:w-5 sm:h-5 shrink-0 ${variantStyles[suggestion.variant] || 'text-slate-600 dark:text-slate-400'}`} />
                  <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Suggestions for you</p>
                </div>
                <h4 className="text-base sm:text-lg font-black text-slate-900 dark:text-slate-100 tracking-tight">{suggestion.headline}</h4>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-400 leading-relaxed">{suggestion.message}</p>
              </div>
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none" />
            </div>
          ) : (
            <div className="flex-1 min-w-0 bg-slate-50 dark:bg-[#0b1222]/80 rounded-xl p-5 sm:p-8 border border-dashed border-slate-200 dark:border-slate-600 flex flex-col items-center justify-center text-center gap-2">
              <p className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                {hasUserPending ? 'Your logs are being audited for compliance' : 'No operational data found for the current cycle'}
              </p>
              <p className="text-[10px] font-medium text-slate-400 dark:text-slate-500 max-w-xs leading-relaxed italic">
                {hasUserPending ? 'Please wait for your supervisor to finalize your grading matrix.' : 'Initiate a log transmission using the Terminal below to activate your performance grading.'}
              </p>
            </div>
          )}
        </div>
