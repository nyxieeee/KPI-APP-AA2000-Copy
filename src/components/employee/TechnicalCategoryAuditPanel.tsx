...
/**
 * Aggregate = sum of each criterion's yield (computed from inputs + admin grading).
...
/** Per-criterion yield for log detail / PDF — same formula as Core audit. */
...
const TechnicalCategoryAuditPanel: React.FC<TechnicalCategoryAuditPanelProps> = ({
  ...
  const renderCriterion = (item: CategoryContentItem, idx: number) => {
    ...
    const basicYieldSections: { label: string; checkpoints: GradingCheckpoint[] }[] | undefined =
    ...
    const renderHeader = () => (
      <div className="flex items-start justify-between gap-4">
        ...
        <YieldBlockAdminStyle
          ...
          basicCheckpointSections={basicYieldSections}
          checkboxN={checkboxN}
      ...
      return (
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 text-[10px] font-bold text-amber-700">No matching controls — enter points manually.</p>
              </div>
            </div>
          ...
          <input
            ...
            min={0}
            max={cap}
            value={typeof task.score === 'number' ? task.score : ''}
            onChange={(e) => {
