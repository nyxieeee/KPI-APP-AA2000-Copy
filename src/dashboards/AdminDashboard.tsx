...
function withDefaultCriterionUi(item: CategoryContentItem): CategoryContentItem {
  ...
  return {
    ...
    ui: {
      definition,
      elements: [
        { type: 'textboxButton', title: label, value: 0 },
        { type: 'basicGradingSystem', checkpoints: [{ min: 0, max: null, score: maxPoints }] },
      ],
    },
  };
}
...
/** Criterion max points: single textbox / checkbox = max row score; multiple textboxes = sum of per-textbox max row scores (cap 1000). */
...
const AdminDashboard: React.FC<Props> = ({
  ...
  /** Criterion max points follow the highest checkpoint score (pts). */
  useLayoutEffect(() => {
  ...
  const DEFAULT_CATEGORY_CONTENT: Record<string, Record<string, { label: string; maxPoints: number }[]>> = {
    Technical: {
      'Project Execution Quality': [
        { label: 'Zero Back-Job Rate', maxPoints: 50 },
        { label: 'First-Time Fix Quality', maxPoints: 25 },
        { label: 'Technical Compliance & Standards', maxPoints: 15 },
        { label: 'Schedule Adherence', maxPoints: 10 },
      ],
      'Client Satisfaction & Turnover': [
        { label: 'Client Satisfaction Score - CSAT', maxPoints: 50 },
        { label: 'Smooth Turnover Rate', maxPoints: 30 },
        { label: 'Zero Client Complaints/Escalations', maxPoints: 20 },
      ],
      'Team Leadership & Accountability': [
        { label: 'Team Performance Under Supervision', maxPoints: 40 },
        { label: 'Safety Record - Zero Incidents', maxPoints: 35 },
        { label: 'Accountability & Ownership', maxPoints: 25 },
      ],
      'Sales Support & Lead Development': [
        { label: 'Site Visits & Technical Consultations', maxPoints: 40 },
        { label: 'Technical Feasibility Confirmations', maxPoints: 35 },
        { label: 'Sales Team Feedback', maxPoints: 25 },
      ],
      'Administrative Excellence': [
        { label: 'Report Submission Timeliness', maxPoints: 60 },
        { label: 'Report Accuracy', maxPoints: 40 },
      ],
    ...
    Sales: {
      'Revenue Score': [{ label: 'Revenue target achievement', maxPoints: 100 }],
      'Accounts Score': [{ label: 'Accounts closed', maxPoints: 100 }],
      'Activities Score': [{ label: 'Sales activities', maxPoints: 100 }],
      'Quotation Mgmt': [{ label: 'Quotation management', maxPoints: 100 }],
    ...
    Accounting: {
      'Accounting Excellence': [
        ...
        { label: 'Accounts Receivable aging within acceptable limits', maxPoints: 17 },
        { label: 'Bank reconciliations completed without discrepancies', maxPoints: 17 },
        { label: 'Ledger entries verified for accuracy', maxPoints: 17 },
        { label: 'Month-end closing procedures completed on schedule', maxPoints: 16 },
        { label: 'Compliance with internal accounting standards', maxPoints: 16 },
      ],
      'Purchasing Excellence': [
        { label: 'Cost savings targets met or exceeded', maxPoints: 17 },
        { label: 'Vendor performance and quality evaluated', maxPoints: 17 },
        { label: 'Purchase Orders processed within SLA', maxPoints: 17 },
        { label: 'Alternative suppliers sourced for better rates', maxPoints: 17 },
        { label: 'Inventory levels optimized to reduce holding costs', maxPoints: 16 },
        { label: 'Procurement policies strictly followed', maxPoints: 16 },
      ],
      'Administrative Excellence': [
        { label: 'Assigned administrative tasks completed on time', maxPoints: 17 },
        { label: 'Documentation and filing systems maintained', maxPoints: 17 },
        { label: 'Service Level Agreements (SLAs) met consistently', maxPoints: 17 },
        { label: 'Data entry accuracy verified', maxPoints: 17 },
        { label: 'Internal communications handled promptly', maxPoints: 16 },
        { label: 'Office supplies and resources managed efficiently', maxPoints: 16 },
      ],
  ...
  }, [gradingEditDept, gradingEditDraftOverride]);
  ...
  const handleUpdateDepartmentWeight = (dept: string, categoryIndex: number, value: number) => {
    ...
    setGradingEditDraft(prev => {
      ...
      list[categoryIndex] = { ...list[categoryIndex], weightPct: clamped };
      return list;
    });
  };
  ...
  const handleResetCategory = (dept: string, categoryIndex: number) => {
    ...
    const defaultContent =
      DEFAULT_CATEGORY_CONTENT[dept]?.[d.label]?.map((c) => withDefaultCriterionUi({ ...c })) ?? undefined;
    setGradingEditDraft(prev => {
      const list = [...(prev || [])];
      if (!list[categoryIndex]) return prev;
                ...
                {rows.map(row => {
                  ...
                  return (
                    <div key={row.name} className="grid grid-cols-[1fr_auto_auto_auto] gap-0 items-center px-5 py-2 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                      ...
                      <div className="text-center px-4 text-sm font-black text-slate-400 tabular-nums">
                        {row.last != null ? `${row.last}%` : '—'}
                      </div>
                      <div className={`text-center px-4 text-sm font-black tabular-nums ${
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          ...
                          <span className={`px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-wide ${scoreBg}`}>
                            {finalScore == null ? 'Score: —' : `Score: ${finalScore}%`}
                          </span>
                        </div>
                      </div>
  ...
  return (
    <div className="w-full max-w-full xl:max-w-[1600px] 2xl:max-w-[1800px] mx-auto px-4 sm:px-6 md:px-8 flex flex-col md:flex-row gap-6 md:gap-8 pb-6 md:pb-12 min-h-0 flex-1 overflow-auto">
      ...
      {gradingEditDept && (() => {
        ...
        const areAllCategoryContentsValid = (categories || []).length > 0 && categoryContentPointSums.every((sum) => sum === 100);
                    ...
                    title={
                      canCommitGrading
                        ...
                        : !isTotalWeightValid
                            ...
                            : 'Changes cannot be confirmed until grading content totals 100 points for every audit category.'
                    }
                    onClick={() => {
                      if (
                        gradingEditDraft &&
                                ...
                                {multiTextboxGrading ? (
                                      ...
                                      return (
                                        <div key={ord} className="rounded-lg border border-slate-100 bg-slate-50/40 p-4 space-y-3">
                                          ...
                                          <div className="hidden sm:grid gap-2 text-[9px] font-black text-slate-400 uppercase tracking-wide px-1 mb-1 sm:grid-cols-12">
                                            ...
                                            <span className="sm:col-span-2">Score (pts)</span>
                                  ...
                                  <>
                                    ...
                                    >
                                      ...
                                      <span className={mode === 'checkbox' ? 'sm:col-span-2' : 'sm:col-span-2'}>Score (pts)</span>
                                      {mode !== 'checkbox' && (
                                        <span className="sm:col-span-3 text-center">No max</span>
                                      )}
                                          ...
                                          >
                                            ...
                                            <div className="sm:col-span-2 space-y-1">
                                              ...
                                              <input
                                                ...
                                                title="Highest score among all rows becomes the criterion max points (Yield)."
                                                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-black text-slate-900 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600"
                                              />
                                            </div>
                                            {mode !== 'checkbox' && (
              ...
              <div className="p-6 overflow-y-auto flex-1 min-h-0">
                ...
                <div className="space-y-4">
                  ...
                  {categories.map((cat, idx) => {
                    ...
                    const isContentPointSumValid = contentPointSum === 100;
                    return (
                      <div
                        key={idx}
