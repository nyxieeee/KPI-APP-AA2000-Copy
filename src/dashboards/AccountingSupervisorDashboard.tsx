...
const AccountingSupervisorDashboard: React.FC<Props> = ({ 
  ...
  const accountingClassificationRows = useMemo(() => {
    ...
    return Object.keys(ACCOUNTING_DEFAULT_WEIGHTS).map(label => ({
      name: label,
      weight: `${(ACCOUNTING_DEFAULT_WEIGHTS[label] * 100).toFixed(0)}%`,
      icon: CATEGORY_ICONS[label] || Calculator
    }));
  }, [departmentWeights]);
  ...
  const handleOpenValidation = (item: Transmission, readOnly: boolean = false) => {
    ...
    // Fallback logic: use configured labels
    if (!snapshot?.length) {
  ...
  }, [grading, accountingWeights, departmentWeights, incentiveTiers]);
  ...
  const handleAction = (type: 'APPROVE' | 'REJECT' | 'OVERRIDE') => {
    ...
    const logDetailSnapshot = labels.map((name) => {
      ...
      if (departmentWeights?.Accounting?.length) {
        weightPct = departmentWeights.Accounting.find((c) => c.label === name)?.weightPct ?? 0;
      } else {
        const legacyKey = ACCOUNTING_LABEL_TO_KEY[name];
        if (legacyKey) weightPct = Math.round((accountingWeights[legacyKey] ?? 0) * 100);
      }
      ...
      <div className="bg-slate-100 dark:bg-[#0b1222] rounded-xl p-5 border border-slate-200 dark:border-slate-600 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className={`text-[10px] font-bold uppercase tracking-wide mb-4 ${queueTab === 'pending' ? 'text-blue-600/90' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>Accounting Department</p>
          ...
          >
            ...
            <p className={`text-[10px] font-bold uppercase tracking-wide mb-4 ${queueTab === 'history' ? 'text-emerald-600/90' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>Accounting Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="var(--ring-track)" strokeWidth="10" />
          ...
          >
            ...
            <p className={`text-[10px] font-bold uppercase tracking-wide mb-4 ${queueTab === 'rejected' ? 'text-red-600/90' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>Accounting Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="var(--ring-track)" strokeWidth="10" />
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-100 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide">Accounting Unit Matrix</h3>
        ...
        <div className="bg-slate-100 dark:bg-[#0b1222] rounded-xl p-5 border border-slate-200 dark:border-slate-600 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              ...
              <h2 className="mt-1 text-xl font-black text-slate-900 dark:text-slate-100 tracking-tight uppercase">Accounting Performance</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-[#0b1222]/50 border-b border-slate-100 dark:border-slate-700">
                  <th className="px-6 py-2 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase tracking-wide">Accounting Personnel</th>
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
          <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide mb-6">Accounting Department — Last 30 Days</h3>
          ...
          <div className="p-5 space-y-12">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              <div className="lg:col-span-7">
                <div className="bg-[#0b1222] rounded-xl p-5 text-white shadow-sm relative overflow-hidden h-full">
                  ...
                  <div className="mt-12 pt-10 border-t border-white/10 relative z-10 flex items-center justify-between">
                    <div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide">Employee Narrative</p>
              </div>
              <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
                <p className="text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400 leading-relaxed italic">"{selectedItem.projectReport || 'No narrative provided for this transmission.'}"</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide">Supervisor Justification</p>
              </div>
              <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
            ...
            <TechnicalLogDetailAuditReview
              ...
              departmentWeights={departmentWeights}
              CLASSIFICATIONS={accountingClassificationRows}
              CHECKLIST_CONTENT={{}} // Accounting uses dynamic criteria primarily
  ...
  return (
    <div className="w-full flex flex-col px-4 sm:px-6 md:px-8 lg:px-8 pb-6 md:pb-12">
      ...
      >
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight leading-none">Accounting Supervisor</h1>
        </div>
