...
const AccountingSupervisorDashboard: React.FC<Props> = ({ 
  user, 
  ...
  const dept = user.department || 'Accounting';
  ...
  const accountingClassificationRows = useMemo(
    () =>
      departmentWeights?.Accounting?.length
        ? departmentWeights.Accounting.map((c) => ({ name: c.label, weight: `${c.weightPct}%` }))
        : [
            { name: 'Accounting Excellence', weight: '40%' },
            { name: 'Purchasing Excellence', weight: '30%' },
            { name: 'Administrative Excellence', weight: '25%' },
            { name: 'Additional Responsibilities', weight: '3%' },
            { name: 'Attendance & Discipline', weight: '2%' },
          ],
    [departmentWeights]
  );
  ...
  const accountingEmployeeCount = deptMembers.length;
  ...
  const getWeightedKpiForEntry = (item: Transmission): number => {
    ...
    return Math.round(
      getAccountingWeightedKpiSum(item, departmentWeights, ACCOUNTING_CHECKLIST_CONTENT, accountingClassificationRows)
    );
  };
  ...
  const handleOpenValidation = (item: Transmission, readOnly: boolean = false) => {
    ...
    if (item.ratings && item.ratings.accountingMetrics) {
      const m = item.ratings.accountingMetrics as any;
      const g = {
        accountingScore: m.accountingScore ?? m.auditScore ?? 0,
        purchasingScore: m.purchasingScore ?? (m.taxScore != null && m.apArScore != null && m.budgetScore != null ? Math.round((m.taxScore + m.apArScore + m.budgetScore) / 3) : 0),
        adminScore: m.adminScore ?? (m.taxScore != null && m.apArScore != null && m.budgetScore != null ? Math.round((m.taxScore + m.apArScore + m.budgetScore) / 3) : 0),
        additionalRespScore: m.additionalRespScore ?? 0,
        attendanceScore: m.attendanceScore ?? 0
      };
      setGrading(g);
      initialGradingRef.current = JSON.parse(JSON.stringify(g));
    } else {
  ...
  const calculatedScore = useMemo(() => {
    const total = 
      (grading.accountingScore * (accountingWeights.accountingScore ?? 0.40)) +
      (grading.purchasingScore * (accountingWeights.purchasingScore ?? 0.30)) +
      (grading.adminScore * (accountingWeights.adminScore ?? 0.25)) +
      (grading.additionalRespScore * (accountingWeights.additionalRespScore ?? 0.03)) +
      (grading.attendanceScore * (accountingWeights.attendanceScore ?? 0.02));
  ...
  }, [grading, accountingWeights, incentiveTiers]);
  ...
  const handleAction = (type: 'APPROVE' | 'REJECT' | 'OVERRIDE') => {
    ...
    if (type === 'REJECT') {
      ...
      const gradingChanged = initial && JSON.stringify(grading) !== JSON.stringify(initial);
    ...
    const logDetailSnapshot = ACCOUNTING_LOG_DETAIL_NAMES.map(({ name, key }) => ({
      name,
      weightPct: Math.round((accountingWeights[key] ?? 0) * 100),
  ...
  }, [setMobileNavConfig, currentPage, pendingCount]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(59 130 246)" strokeWidth="10" strokeLinecap="round" strokeDasharray={2 * Math.PI * 42} strokeDashoffset={2 * Math.PI * 42 * (1 - (totalDeptAudits ? Math.min(1, pendingCount / totalDeptAudits) : 0))} className="transition-[stroke-dashoffset] duration-500 animate-ring-pulse" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          >
            ...
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(226 232 240)" strokeWidth="10" />
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(16 185 129)" strokeWidth="10" strokeLinecap="round" strokeDasharray={2 * Math.PI * 42} strokeDashoffset={2 * Math.PI * 42 * (1 - (totalDeptAudits ? Math.min(1, validatedCount / totalDeptAudits) : 0))} className="transition-[stroke-dashoffset] duration-500 animate-ring-pulse" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          >
            ...
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(226 232 240)" strokeWidth="10" />
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(239 68 68)" strokeWidth="10" strokeLinecap="round" strokeDasharray={2 * Math.PI * 42} strokeDashoffset={2 * Math.PI * 42 * (1 - (totalDeptAudits ? Math.min(1, rejectedCount / totalDeptAudits) : 0))} className="transition-[stroke-dashoffset] duration-500 animate-ring-pulse" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            return (
              <div key={item.id} className="group flex items-center justify-between p-6 rounded-lg border bg-white border-slate-50 hover:shadow-md transition-all">
                ...
                <div className="flex items-center gap-3">
                  {queueTab === 'pending' && isPendingGradingConfigExpired(item, 'Accounting', departmentWeights) ? (
                    <GradingExpiredBadge />
                  ) : null}
                  {(() => {
  ...
  const renderIncentives = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <SupervisorIncentiveMatrixPanel tiers={incentiveTiers} departmentLabel="Accounting" />
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="px-6 py-2 text-[10px] font-black text-slate-400 uppercase tracking-wide">Accounting Personnel</th>
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-wide mb-6">Accounting Department — Last 30 Days</h3>
  ...
  const renderValidation = () => {
    ...
    const accData = selectedItem.allSalesData || {};
    const auditData = accData['Financial Audit & Compliance'] || {};
    const taxData = accData['Taxation & Statutory Reporting'] || {};
    const apArData = accData['Accounts Payable/Receivable'] || {};
          ...
          <div className="p-5 space-y-12">
                ...
                <div className="bg-[#0b1222] rounded-xl p-5 text-white shadow-sm relative overflow-hidden h-full">
                  ...
                  <div className="flex items-center justify-between mb-8 relative z-10">
                    <div className="flex items-center gap-4">
                      ...
                      <h4 className="text-sm font-black uppercase tracking-wide">Accounting Grading Matrix</h4>
                    </div>
                    {!isReadOnly && (
                      <button 
                      ...
                      return (
                        <div key={category} className="space-y-2">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-xs font-black text-white">{grading[key]} PTS</span>
                          </div>
                            ...
                            {[1, 2, 3, 4, 5].map((val) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex-1 h-2 rounded-full transition-all ${scoreVal <= grading[key] ? 'bg-blue-600' : 'bg-white/10'} ${isReadOnly ? 'cursor-default' : ''}`}
                                  aria-label={`${category} set to ${scoreVal}`}
                                  type="button"
                                />
                              );
                            })}
              ...
              <div className="lg:col-span-5 flex flex-col gap-6 min-h-0">
                {!isReadOnly ? (
                  ...
                  <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm space-y-6 h-full flex flex-col justify-center">
                    ...
                    <button type="button" onClick={() => setCurrentPage('queue')} className="w-full py-2 bg-slate-900 text-white rounded-lg text-[11px] font-black uppercase tracking-wide transition-all mt-6">Back to List</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-wide">Employee Narrative</p>
              </div>
              <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                <p className="text-sm font-medium text-slate-600 leading-relaxed italic">"{selectedItem.projectReport || 'No narrative provided for this transmission.'}"</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-wide">Supervisor Justification</p>
              </div>
              <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-wide">Detailed Audit Review</p>
              </div>
              ...
              {(() => {
                ...
                const normalizeCategoryName = (name: string) => {
                  if (!name) return name;
                  if (name === 'Attendance') return 'Attendance & Discipline';
                  if (name === 'Additional Responsibility') return 'Additional Responsibilities';
                  return name;
                };
                ...
                // IMPORTANT: Detailed Audit Review must reflect what was submitted (stored payload),
                // and for totals/weights it should prefer the validation snapshot when present.
                const snapshot = selectedItem.ratings?.logDetailSnapshot || [];
                const weights = departmentWeights?.Accounting || [];
                const categories =
                  (weights.length ? weights.map((w) => normalizeCategoryName(w.label)) : []) ||
                  [];
                ...
                const fallbackCategories =
                  categories.length > 0
                    ? categories
                    : snapshot.length > 0
                      ? snapshot.map((s) => normalizeCategoryName(s.name))
                      : Object.keys(selectedItem.allSalesData || {}).map(normalizeCategoryName);

                const getSnapshotRow = (category: string) =>
                  snapshot.find((s) => normalizeCategoryName(s.name) === category);

                const getWeightPct = (category: string) => {
                  const snap = getSnapshotRow(category);
                  if (snap) return snap.weightPct;
                  const w = weights.find((x) => normalizeCategoryName(x.label) === category);
                  return w?.weightPct ?? 0;
                };
                ...
                return (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {fallbackCategories.map((category) => {
                      const Icon = CATEGORY_ICONS[category] || Calculator;
                      const weightPct = getWeightPct(category);

                      const weightItem = weights.find((w) => normalizeCategoryName(w.label) === category);
                      const rawPayload = selectedItem.allSalesData || {};
                      const payload =
                        rawPayload[category] ??
                        rawPayload[normalizeCategoryName(category)] ??
                        rawPayload[Object.keys(rawPayload).find((k) => normalizeCategoryName(k) === category) || ''];
                      ...
                      // Prefer snapshot score when present; otherwise compute from submitted payload panels.
                      const snap = getSnapshotRow(category);
                      const totalScoreFromPayload = (() => {
                        if (!obj) return 0;
                        if (category === 'Accounting Excellence') {
                          return (
                            safeNum(obj.accExFinancial) +
                            safeNum(obj.accExTimeliness) +
                            safeNum(obj.accExAR) +
                            safeNum(obj.accExReconciliation)
                          );
                        }
                        if (category === 'Purchasing Excellence') {
                          return safeNum(obj.purchasingCostSavings) + safeNum(obj.purchasingVendorMgmt) + safeNum(obj.purchasingPOAccuracy);
                        }
                        if (category === 'Administrative Excellence') {
                          return safeNum(obj.adminTaskSLA) + safeNum(obj.adminAccuracyQuality) + safeNum(obj.adminCustomerSatisfaction);
                        }
                        if (category === 'Attendance & Discipline') {
                          const att = (obj.attendance || {}) as any;
                          const days = safeNum(att.days);
                          const late = safeNum(att.late);
                          const violations = safeNum(att.violations);
                          const absenceScore = Math.max(0, 60 - 10 * days);
                          const punctualityScore = Math.max(0, 30 - 5 * Math.max(0, late - 3));
                          const disciplineScore = Math.max(0, 10 - 10 * violations);
                          return absenceScore + punctualityScore + disciplineScore;
                        }
                        if (category === 'Additional Responsibilities') return safeNum(obj.additionalRespValue);
                        return 0;
                      })();

                      const totalScore = typeof snap?.score === 'number' ? snap.score : totalScoreFromPayload;
                      ...
                      const renderTasks = () => {
                        ...
                        if (category === 'Accounting Excellence') {
                          panels.push({
                            label: 'Financial Accuracy & Compliance',
                            maxPoints: 35,
                            score: safeNum(obj.accExFinancial),
                            details: [
                              { label: 'Critical errors', value: safeNum(obj.accExFinancialCritical) },
                              { label: 'Minor errors', value: safeNum(obj.accExFinancialMinor) },
                            ],
                          });
                          panels.push({
                            label: 'Timeliness — Reports & Entries',
                            maxPoints: 25,
                            score: safeNum(obj.accExTimeliness),
                            details: [
                              { label: 'Report day', value: safeNum(obj.accExReportsDay) },
                              { label: 'Entry %', value: safeNum(obj.accExEntryPct) },
                            ],
                          });
                          panels.push({
                            label: 'Accounts Receivable Management',
                            maxPoints: 25,
                            score: safeNum(obj.accExAR),
                            details: [
                              { label: 'DSO days', value: safeNum(obj.accExDSODays) },
                              { label: 'Collections %', value: safeNum(obj.accExCollectionsPct) },
                            ],
                          });
                          panels.push({
                            label: 'Reconciliation & Month-End Close',
                            maxPoints: 15,
                            score: safeNum(obj.accExReconciliation),
                            details: [{ label: 'Close day', value: safeNum(obj.accExCloseDay) }],
                          });
                        } else if (category === 'Purchasing Excellence') {
                          panels.push({
                            label: 'Cost Savings & Budget Compliance',
                            maxPoints: 40,
                            score: safeNum(obj.purchasingCostSavings),
                            details: [
                              { label: 'Cost savings %', value: safeNum(obj.purchasingCostSavingsPct) },
                              { label: 'Budget over %', value: safeNum(obj.purchasingBudgetOverPct) },
                            ],
                          });
                          panels.push({
                            label: 'Vendor Management & Quality',
                            maxPoints: 35,
                            score: safeNum(obj.purchasingVendorMgmt),
                            details: [
                              { label: 'Vendor rating', value: safeNum(obj.purchasingVendorRating) },
                              { label: 'Due diligence incomplete', value: safeNum(obj.purchasingDueDiligenceIncomplete) },
                              { label: 'Stock incidents', value: safeNum(obj.purchasingStockIncidents) },
                            ],
                          });
                          panels.push({
                            label: 'PO Processing & Accuracy',
                            maxPoints: 25,
                            score: safeNum(obj.purchasingPOAccuracy),
                            details: [
                              { label: 'PO speed %', value: safeNum(obj.purchasingPOSpeedPct) },
                              { label: 'PO accuracy %', value: safeNum(obj.purchasingPOAccuracyPct) },
                            ],
                          });
                        } else if (category === 'Administrative Excellence') {
                          panels.push({
                            label: 'Task Completion & SLA',
                            maxPoints: 45,
                            score: safeNum(obj.adminTaskSLA),
                            details: [
                              { label: 'On-time %', value: safeNum(obj.adminOnTimePct) },
                              { label: 'SLA met %', value: safeNum(obj.adminSLAMetPct) },
                            ],
                          });
                          panels.push({
                            label: 'Accuracy & Quality',
                            maxPoints: 35,
                            score: safeNum(obj.adminAccuracyQuality),
                            details: [
                              { label: 'Error rate %', value: safeNum(obj.adminErrorRatePct) },
                              { label: 'Accuracy %', value: safeNum(obj.adminAccuracyPct) },
                            ],
                          });
                          panels.push({
                            label: 'Internal Customer Satisfaction',
                            maxPoints: 20,
                            score: safeNum(obj.adminCustomerSatisfaction),
                            details: [{ label: 'Satisfaction', value: safeNum(obj.adminSatisfactionPct) }],
                          });
                        } else if (category === 'Attendance & Discipline') {
                          const att = (obj.attendance || {}) as any;
                          const days = safeNum(att.days);
                          const late = safeNum(att.late);
                          const violations = safeNum(att.violations);
                          ...
                          panels.push({
                            ...
                            maxPoints: 60,
                            score: absenceScore,
                            details: [{ label: 'Absences', value: days }],
                          });
                          panels.push({
                            label: 'Punctuality',
                            maxPoints: 30,
                            score: punctualityScore,
                            details: [{ label: 'Lates', value: late }],
                          });
                          panels.push({
                            label: 'Discipline',
                            maxPoints: 10,
                            score: disciplineScore,
                            details: [{ label: 'Violations', value: violations }],
                          });
                        } else if (category === 'Additional Responsibilities') {
                          panels.push({
                            label: 'Additional Responsibilities',
                            maxPoints: 100,
                            score: safeNum(obj.additionalRespValue),
                          });
                        }

                        // Fallback: if we couldn't map to panels, show raw payload (no invented labels).
                        if (panels.length === 0) {
                          ...
                          return entries
                            .map(([k, v]) => {
                              ...
                              if (v && typeof v === 'object' && !Array.isArray(v)) {
                                const nestedEntries = Object.entries(v as Record<string, any>).slice(0, 6);
                                return (
                                  <div key={k} className="bg-slate-50 p-5 rounded-lg border border-slate-100 flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
                                    <div className="flex justify-between items-start mb-2">
                        ...
                        return panels.map((panel) => (
                          <div key={panel.label} className="bg-slate-50 p-5 rounded-lg border border-slate-100 flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
                            <div>
                              <div className="flex justify-between items-start mb-2">
                                <span className="text-[11px] font-black text-slate-700 uppercase tracking-tight leading-tight">{panel.label}</span>
                                <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${panel.score === panel.maxPoints ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>
                                  {panel.score} / {panel.maxPoints}
                                </span>
                              </div>
                              {panel.details?.length ? (
                                <div className="space-y-1.5">
                                  {panel.details.map((row) => (
                                    <div key={row.label} className="flex justify-between text-[10px] text-slate-500 gap-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-wide">Attachments</p>
              </div>
...
export default AccountingSupervisorDashboard;
