...
const SalesSupervisorDashboard: React.FC<Props> = ({ 
  ...
  const calculatedScore = useMemo(() => {
    let total = 0;
    supervisorSalesClassificationRows.forEach(row => {
      const weight = (parseInt(row.weight, 10) || 0) / 100;
      const key = SALES_LABEL_TO_KEY[row.name];
      const score = key ? (grading[key] ?? 0) : 0;
      total += score * weight;
    });
  ...
  }, [grading, supervisorSalesClassificationRows, incentiveTiers]);
  ...
  const handleAction = (type: 'APPROVE' | 'REJECT' | 'OVERRIDE') => {
    ...
    const logDetailSnapshot = supervisorSalesClassificationRows.map(row => {
      const key = SALES_LABEL_TO_KEY[row.name];
      return {
        name: row.name,
        label: row.name,
        score: key ? (grading[key] ?? 0) : 0,
        weightPct: parseInt(row.weight, 10) || 0
      };
    });
      ...
      <div className="bg-slate-100 dark:bg-[#0b1222] rounded-xl p-5 border border-slate-200 dark:border-slate-600 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className={`text-[10px] font-bold uppercase tracking-wide mb-4 ${queueTab === 'pending' ? 'text-blue-600/90' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>Sales Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-2xl font-black tabular-nums leading-none ${queueTab === 'pending' ? 'text-blue-700 dark:text-blue-400' : 'text-blue-600'}`}>{salesPendingCount}/{salesTotalAudits}</span>
          ...
          <button
            ...
            className={`w-full p-5 rounded-lg border flex flex-col items-center text-left transition-transform duration-200 ease-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-slate-100 dark:focus:ring-offset-slate-800 hover:scale-[1.02] hover:-translate-y-0.5 hover:shadow-lg ${queueTab === 'history' ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-300 dark:border-blue-600 shadow-md scale-[1.06] -translate-y-1' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-900'}`}
            aria-label="Show validated audits in registry"
            aria-pressed={queueTab === 'history'}
          >
            <p className={`text-[10px] font-black uppercase tracking-wide mb-1 ${queueTab === 'history' ? 'text-blue-700 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500 dark:text-slate-500'}`}>Approved</p>
            <p className={`text-[10px] font-bold uppercase tracking-wide mb-4 ${queueTab === 'history' ? 'text-blue-600/90' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>Sales Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="var(--ring-track)" strokeWidth="10" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-2xl font-black tabular-nums leading-none ${queueTab === 'history' ? 'text-blue-700 dark:text-blue-400' : 'text-blue-600'}`}>{salesValidatedCount}/{salesTotalAudits}</span>
                <span className={`text-[10px] font-bold uppercase tracking-wide mt-1 ${queueTab === 'history' ? 'text-blue-600' : 'text-slate-400 dark:text-slate-500 dark:text-slate-500'}`}>validated</span>
              </div>
            </div>
          </button>
          <button
          ...
          >
            ...
            <p className={`text-[10px] font-bold uppercase tracking-wide mb-4 ${queueTab === 'rejected' ? 'text-red-600/90' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>Sales Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="var(--ring-track)" strokeWidth="10" />
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      ...
                      {!(queueTab === 'rejected') && (
                        <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md border w-fit ${
                          item.status === 'validated' ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-100 dark:border-blue-900/50 text-blue-600' :
                          item.status === 'rejected' ? 'bg-red-50 dark:bg-red-900/30 border-red-100 text-red-600' :
                          'bg-amber-50 dark:bg-amber-900/30 border-amber-100 dark:border-amber-800/50 text-amber-600'
                        }`}>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-100 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide">Sales Unit Matrix</h3>
        ...
        <div className="bg-slate-100 dark:bg-[#0b1222] rounded-xl p-5 border border-slate-200 dark:border-slate-600 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              ...
              <h2 className="mt-1 text-xl font-black text-slate-900 dark:text-slate-100 tracking-tight uppercase">Sales Performance</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-[#0b1222]/50 border-b border-slate-100 dark:border-slate-700">
                  <th className="px-6 py-2 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase tracking-wide">Sales Personnel</th>
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
          <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide mb-6">Sales Department — Last 30 Days</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="p-6 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-100 dark:border-blue-900/50">
          ...
          <div className="p-5 space-y-12">
                      ...
                      <div key={cat.key} className="space-y-2">
                        <div className="flex justify-between items-center">
                           <div className="flex items-center gap-2">
                             <cat.icon className="w-3 h-3 text-slate-400 dark:text-slate-500 dark:text-slate-500" />
                ...
                {!isReadOnly ? (
                  <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-100 dark:border-slate-700 shadow-sm h-full flex flex-col min-h-0">
                    <div className="flex flex-col flex-1 min-h-0 gap-4">
                      <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-500 uppercase tracking-wide ml-1 flex-shrink-0">Supervisor Comments</label>
                      <div className="flex-1 min-h-0 flex flex-col">
                        <textarea 
                          ...
                          className="w-full h-full min-h-[140px] bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg p-5 text-sm font-medium text-slate-700 dark:text-slate-300 outline-none focus:ring-4 focus:ring-blue-500/10 resize-none overflow-auto"
            ...
            {selectedItem.projectReport && (
              <div className="space-y-4">
                <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center"><Megaphone className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-wide">Employee Narrative</p></div>
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-5 text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400 italic border border-blue-50 leading-relaxed">"{selectedItem.projectReport}"</div>
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-5 text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400 italic border border-blue-50 leading-relaxed">"{selectedItem.supervisorComment || (isReadOnly ? 'No supervisor justification recorded.' : 'Not validated yet.')}"</div>
            </div>
