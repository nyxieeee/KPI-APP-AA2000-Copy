...
const DEFAULT_TECH_CATEGORY_LABELS = [
  'Project Execution Quality',
  'Client Satisfaction & Turnover',
  'Team Leadership & Accountability',
  'Attendance & Discipline',
...
const TechnicalSupervisorDashboard: React.FC<Props> = ({ 
  ...
  /** Matches employee Technical registry / `getTechnicalWeightedKpiSum` classification names. */
  ...
  const matrixCategoryLabels = useMemo(
    () => departmentWeights?.Technical?.map((c) => c.label) ?? DEFAULT_TECH_CATEGORY_LABELS,
    [departmentWeights]
  );
  ...
  const techLogDetailClassifications = useMemo(() => {
    return matrixCategoryLabels.map((name) => {
      ...
      const catCfg = departmentWeights?.Technical?.find((c) => c.label === name);
  ...
  const getReviewTotalScoreLegacyTechnical = useCallback(
    (category: string, checklist: unknown) => {
      const catCfg = departmentWeights?.Technical?.find((w) => w.label === category);
  ...
  const filteredPending = useMemo(() => {
    // Department bucket is already scoped — no need to re-filter by roleMap
    // which could silently drop items if the employee is missing from registry.
  ...
  const filteredHistory = useMemo(() => {
    // Department bucket is already scoped — pass through all items.
  ...
  const getWeightedKpiForEntry = (item: Transmission): number => {
    return Math.round(
      getTechnicalWeightedKpiSum(item, departmentWeights, CHECKLIST_CONTENT, supervisorClassificationRows)
    );
  };
  ...
  const handleOpenValidation = (item: Transmission, readOnly: boolean = false) => {
    ...
    // Bug 1 fix: Prefer logDetailSnapshot scores (saved at grading/submission time)
    // so supervisor sees the same scores as the employee, not recalculated ones.
    ...
    // Fill any remaining labels from raw recalculation
                ...
                <div className="bg-[#0b1222] rounded-xl p-5 text-white shadow-sm relative overflow-hidden h-full">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 relative z-10">
                    {matrixCategoryLabels.map((category) => {
                      const catCfg = departmentWeights?.Technical?.find((c) => c.label === category);
  ...
  return (
    <div className="w-full flex flex-col pb-8">
      ...
      >
        <div>
          <h1 className="text-lg font-black text-slate-900 dark:text-slate-100 tracking-tight leading-none">Technical Supervisor</h1>
        </div>
      </div>
